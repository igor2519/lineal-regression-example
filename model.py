...
def save_initial_datasets(X: ArrayLike, y: ArrayLike,X_filename:str = "X.joblib",y_filename:str= "Y.joblib"):
    """
    ...
    dump(X, X_filename)
    print(f"Feature matrix saved to {X_filename}")
    
    dump(y, y_filename)
    print(f"Target values saved to {y_filename}")
...
if __name__ == '__main__':
    ...
    # Save the model
    save_regression_model(model,'./sources/linear_regression_model.joblib')

    # Save datasets
    save_initial_datasets(X, y,"./sources/X.joblib","./sources/Y.joblib")